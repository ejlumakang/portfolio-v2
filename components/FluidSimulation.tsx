...
export default function FluidSimulation({
  ...
  useEffect(() => {
    ...
    function getSupportedFormat(gl: WebGL2RenderingContext | WebGLRenderingContext, internalFormat: number, format: number, type: number): { internalFormat: number; format: number } | null {
    ...
    function updateFrame() {
      ...
      // Optimization: Only compute physics transitions if the system isn't completely stale/idle
      const isIdle = Date.now() - lastActivityTime.current > 2500 // 2.5 seconds idle buffer
      ...
      if (!isIdle) {
        ...
        // Keeps WebGL background drawing cleanly while resting, but skips math iterations completely
        render(null)
      }
    ...
    function updatePointerDownData(pointer: Pointer, id: number, posX: number, posY: number): void {
      ...
      pointer.texcoordX = posX / canvasEl.width; pointer.texcoordY = 1.0 - posY / canvasEl.height
    ...
    function generateColor() {
      if (usePrimaryColors) {
        const primaryColors = [
          { r: 0xfd / 255, g: 0xd1 / 255, b: 0xd9 / 255 }, 
    ...
    function shouldSkipSplash(target: EventTarget | null): boolean {
      ...
      while (element) {
        if (element.hasAttribute('data-skip-splash-cursor')) return true
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      updatePointerDownData(pointer, -1, scaleByPixelRatio(pos.x), scaleByPixelRatio(pos.y))
      clickSplat(pointer)
    }
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        updatePointerDownData(pointer, touches[i].identifier, scaleByPixelRatio(pos.x), scaleByPixelRatio(pos.y))
      }
    }
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const pos = getRelativePosition(touches[i].clientX, touches[i].clientY)
        updatePointerMoveData(pointer, scaleByPixelRatio(pos.x), scaleByPixelRatio(pos.y), pointer.color)
      }
    }
    ...
    container.addEventListener('touchend', () => updatePointerDownData(pointers[0], -1, 0, 0))
  ...
  }, [mounted, SIM_RESOLUTION, DYE_RESOLUTION, CAPTURE_RESOLUTION, DENSITY_DISSIPATION, VELOCITY_DISSIPATION, PRESSURE, PRESSURE_ITERATIONS, CURL, SPLAT_RADIUS, SPLAT_FORCE, SHADING, COLOR_UPDATE_SPEED, BACK_COLOR, TRANSPARENT, usePrimaryColors])
  ...
  return (
    <div
      ref={containerRef}
      className={`relative overflow-hidden w-full h-full min-h-screen absolute inset-0 ${containerClassName || ''}`}
    >
      <canvas
        ref={canvasRef}
        id='fluid'
