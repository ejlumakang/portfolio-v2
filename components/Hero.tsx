...
export default function Hero() {
  ...
  return (
    <section id="home" className="min-h-screen flex flex-col justify-center pt-20 pb-12 relative overflow-hidden bg-background">
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 flex flex-col-reverse md:flex-row-reverse items-center justify-center px-6 md:px-12 w-full z-10 max-w-6xl mx-auto gap-12 md:gap-16">
        <div className="w-full md:w-1/2 flex flex-col items-center md:items-start justify-center text-center md:text-left">
          <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-border/40 bg-muted/20 text-xs font-mono tracking-tight text-muted-foreground/80 mb-8">
            <span className="w-1.5 h-1.5 rounded-full bg-pink-500 animate-pulse" />
            Available for Work
          </motion.div>

          <motion.h1 variants={itemVariants} className="text-[12vw] sm:text-[8vw] md:text-[5vw] font-black tracking-tight leading-[1] text-foreground select-none w-full">
            ...
            <br />
            <span className="font-serif italic text-[14vw] sm:text-[10vw] md:text-[7vw] cursor-default transition-colors duration-300 hover:text-pink-600">Lumakang</span>
          </motion.h1>

          <motion.p variants={itemVariants} className="mt-6 text-muted-foreground text-lg max-w-lg">
            A Computer Science student studying Intelligent Systems at De La Salle University-Dasmarinas.
          </motion.p>
          
          <motion.div variants={itemVariants} className="flex flex-col items-center md:items-start gap-6 mt-10">
            ...
            <div className="flex items-center gap-6 text-muted-foreground pt-2">
              <a href="mailto:ejlumakang@gmail.com" className="hover:text-pink-600 transition-colors"><Mail className="w-5 h-5" /></a>
              <a href="https://www.linkedin.com/in/ejlmkng/" className="hover:text-pink-600 transition-colors"><FaLinkedin size={20} /></a>
              <a href="https://github.com/ejlumakang" className="hover:text-pink-600 transition-colors"><FaGithub size={20} /></a>
            </div>
          </motion.div>
        </div>
